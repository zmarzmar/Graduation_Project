...
async def delete_search_history_by_id(db: AsyncSession, history_id: int, user_id: int) -> bool:
    """검색 기록 개별 소프트 딜리트. 본인 소유만 가능. 성공 여부 반환"""
    result = await db.execute(
...
async def delete_all_search_history(db: AsyncSession, user_id: int) -> int:
    """특정 유저의 검색 기록 전체 소프트 딜리트. 처리된 행 수 반환"""
    result = await db.execute(
